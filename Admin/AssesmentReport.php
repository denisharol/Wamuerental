...
$properties = $conn->query("SELECT DISTINCT property FROM users WHERE property IS NOT NULL AND property != ''");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("
        INSERT INTO assesment_report (property, tenant_id, unit, security_deposit, damages, repair_amount, payment_method, remaining_deposit)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->bind_param(
        "sisssdss",
...
<div class="container">
    ...
    <div class="filters">
        ...
        <form method="get">
            ...
            <select name="filter" id="propertyFilter" onchange="filterTenants()">
...
<script>
    ...
    function calculateRemainingDeposit() {
        const repairAmount = parseFloat(document.getElementById("repairAmountField").value) || 0;
        const securityDeposit = parseFloat(document.getElementById("securityDepositField").value) || 0;
        const paymentMethod = document.getElementById("paymentMethodDropdown").value;

        if (paymentMethod === "Security Deposit") {
            document.getElementById("remainingDepositField").value = (securityDeposit - repairAmount).toFixed(2);
        } else {
            document.getElementById("remainingDepositField").value = securityDeposit.toFixed(2);
        }
    }
</script>
</body>
